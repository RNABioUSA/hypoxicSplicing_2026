...
.run_fig1_cluster_pathway_enrichment <- function(
  clusters_df,
  deseq_results,
  annot_datasets = c(
    BP = "GO:0008150",
  ...
  # Figure 1 clusters are defined across the complete time course rather than
  # within one contrast, so use the union of the contrast-specific tested
  # universes as the common reference set for every cluster.
  tested_universe <- unique(unlist(tested_universe_by_tp, use.names = FALSE))
  tested_universe <- tested_universe[
    !is.na(tested_universe) & tested_universe != ""
  ]

  cluster_gene_sets <- split(
    as.character(clusters_df$ensgene),
    paste0("Cluster ", clusters_df$cluster)
  )
  cluster_gene_sets <- lapply(cluster_gene_sets, function(x) {
    x <- unique(x[!is.na(x) & x != ""])
    intersect(x, tested_universe)
  })

  cache_dir <- file.path(outdir, "cache")
  dir.create(cache_dir, recursive = TRUE, showWarnings = FALSE)

  enrichment_by_cluster <- stats::setNames(
    lapply(names(cluster_gene_sets), function(cluster_name) {
      stats::setNames(
        lapply(names(annot_datasets), function(ontology_name) {
          annot_dataset <- unname(annot_datasets[[ontology_name]])
          cluster_id <- gsub("[^A-Za-z0-9]+", "_", tolower(cluster_name))
          ontology_id <- gsub("[: ]", "_", annot_dataset)

          cache_file <- file.path(
            cache_dir,
            paste0(
              "fig1_", cluster_id, "_", ontology_id,
              "_fdr", enrichment_cutoff, ".rds"
            )
          )

          .run_panther_enrich_from_ensgenes(
            query_ensgenes = cluster_gene_sets[[cluster_name]],
            universe_ensgenes = tested_universe,
            annot_df = annot_df,
            organism = 9606,
            ref_organism = 9606,
        ...
        names(annot_datasets)
      )
    }),
    names(cluster_gene_sets)
  )

  complete_list <- list()
  list_index <- 1L

  for (cluster_name in names(enrichment_by_cluster)) {
    for (ontology_name in names(enrichment_by_cluster[[cluster_name]])) {
      enrich_obj <- enrichment_by_cluster[[cluster_name]][[ontology_name]]
      enrich_tbl <- enrich_obj$result

      if (is.null(enrich_tbl) || nrow(enrich_tbl) == 0) {
        next
      }

      enrich_tbl <- as.data.frame(enrich_tbl, stringsAsFactors = FALSE)
      enrich_tbl$cluster <- cluster_name
      enrich_tbl$ontology <- ontology_name
      enrich_tbl$annot_dataset <- unname(annot_datasets[[ontology_name]])

      leading_cols <- c("cluster", "ontology", "annot_dataset")
      enrich_tbl <- enrich_tbl[, c(
        leading_cols,
        setdiff(colnames(enrich_tbl), leading_cols)
      ), drop = FALSE]

      complete_list[[list_index]] <- enrich_tbl
      list_index <- list_index + 1L
    }
  }

  complete_tbl <- if (length(complete_list) == 0) {
    data.frame()
  } else {
    do.call(rbind, complete_list)
  }

  if (nrow(complete_tbl) > 0) {
    rownames(complete_tbl) <- NULL

    sort_cols <- intersect(
      c("cluster", "ontology", "fdr", "pValue", "p.value"),
      colnames(complete_tbl)
    )
    if (length(sort_cols) > 0) {
      complete_tbl <- complete_tbl[do.call(order, complete_tbl[sort_cols]), , drop = FALSE]
      rownames(complete_tbl) <- NULL
    }
  }
  ...
  out <- list(
    meta = list(
      ...
      background_definition = paste(
        "Union of active/tested gene universes for H1, H3, and H24",
        "returned by .get_active_gene_universe_by_tp()."
      ),
      outdir = outdir
    ),
    gene_sets = list(
      cluster_gene_sets = cluster_gene_sets,
    ...
    enrichment = list(
      by_cluster = enrichment_by_cluster,
  ...
  enrichment_rds <- file.path(
    outdir,
    "fig_1_cluster_pathway_enrichment_analysis.rds"
  )
  saveRDS(out, enrichment_rds)

  if (isTRUE(save_tables) && nrow(complete_tbl) > 0) {
    utils::write.table(
      complete_tbl,
      file = file.path(outdir, "fig_1_cluster_pathway_enrichment_complete.tsv"),
      sep = "\t",
      quote = FALSE,
      row.names = FALSE
    )
...
run_gene_level_analysis <- function(
  ...
  if (isTRUE(run_pathway_enrichment)) {
    message("Running Figure 1 cluster-level PANTHER enrichment analysis...")
    pathway_enrichment <- .run_fig1_cluster_pathway_enrichment(
